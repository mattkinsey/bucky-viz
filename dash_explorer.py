...
print('-------------------loading dependancy libraries')
...
def prepare_data_reichlab_bucky(b_path, r_path, m_str):
    ...
    # GET ALL CURRENT BUCKY OUTPUT
    df_b_out = get_reichlab_bucky_output_df(bucky_data_path)
    ...
    if len(df_b_out) == 0:
        ...
        # GET REICHLAB TRUTH  National Data
        ...
        # 7 day rolling SUM / average to smooth weekends
        df_us_ic = df_us_c.copy()
        df_us_ic['value'] = df_us_ic['value'].rolling(7).sum()
        df_us_ic.dropna(how='any', axis=0, inplace=True)
        df_us_id = df_us_d.copy()
        df_us_id['value'] = df_us_id['value'].rolling(7).sum()
        df_us_id.dropna(how='any', axis=0, inplace=True)
        df_us_ih = df_us_h.copy()
        df_us_ih['value'] = df_us_ih['value'].rolling(7).mean()
        df_us_ih.dropna(how='any', axis=0, inplace=True)
...
# DUPLICATED DASH FUNCTION for REICHLAB-BUCKY model
def define_dash_server_bucky(df, b_models, monday_str):
    ...
    def update_line_chart(models, demo, ci):
        #print('test', models, ' - ', demo, ' - ', ci, )
        df = df_b_out#[mask]
        ...
        for i,v in enumerate(models):
            ...
            # SUNDAY MARKERS ------- <<<<  MISSING DATA to complete this
            c0 = np.stack((df_s_lower[input_y], df_s_upper[input_y]),axis=1)
            t0 = str(df_s_mid['quantile'].unique())+' - '+df_hist_state+' '+input_y
            fig.add_trace(go.Scatter(x=df_s_mid['date'], y=df_s_mid[input_y],
                name='.50 Sundays '+df_hist_state+' '+vv,
...
def start_reichlab_bucky_dash():
    print('')
    print('Bucky Model Explorer.')
    print('-----------------')
    import os

    #desktop = os.path.join(os.path.join(os.environ['USERPROFILE']), 'Desktop')
    
    #monday_str = '2022-08-29'  # <-- this date should be within the range provided in the Bucky Model output
                               # Will use the most recent monday if commented out 
    
    reichlab_data_path = '/mnt/c/Users/'+str(Path.home()).split('/')[-1]+'/Desktop/python/covid19-forecast-hub/'
    bucky_data_path = '/mnt/c/Users/'+str(Path.home()).split('/')[-1]+'/Desktop/python/covid19-forecast-hub/data-processed/JHUAPL-Bucky/'+monday_str+'-JHUAPL-Bucky.csv'
    
    
    df, b_models = prepare_data_reichlab_bucky(bucky_data_path, reichlab_data_path, monday_str)
    
    
    if df.size > 0:
        print('Local Data loaded.')
        define_dash_server_bucky(df, b_models, monday_str)
    else:
        print('end.')
    return
...
if __name__ == '__main__':
    ...
    while x not in ok:
        ...
        if x == '1':
            ...
            print('')
            print('Process ReichLab_Bucky data.')
            print('Local data paths may need to be configured in the start_dash() function.')
            print('This process will assume that the most recent Reich Lab data was pulled and auto-configure the target date.')
            print('Data preprocess may take a few seconds.')
            print('If the data paths have not been set to the Reigh Lab data repository, use the Demo data file by choosing option 1.')
            print('')
            y = input('Enter to Continue.')
            start_reichlab_bucky_dash()
